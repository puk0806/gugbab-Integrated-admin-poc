...
app.prepare().then(() => {
  const localAddress = networkInterface["en0"]?.find(
    ({ family }) => family === "IPv4"
  )?.["address"];
  const strings = {
  ...
  http((req, res) => {
    ...
    if (err) throw err;
    console.log(
      `${strings.http}  ${strings.ready} on http://localhost:${ports.http}`
    );
    console.log(
      `${strings.http}  ${strings.ready} on http://${localAddress}:${ports.http}`
    );
  });
  ...
  https(httpsOptions, (req, res) => {
    ...
    if (err) throw err;
    console.log(
      `${strings.https} ${strings.ready} on https://${hostname}:${ports.https}`
    );
    console.log(
      `${strings.https} ${strings.ready} on https://${localAddress}:${ports.https}`
    );
  });
});
